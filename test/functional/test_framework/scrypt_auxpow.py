...
def mineScryptAux (node, chainid, ok):
  ...
  """
  cb_addr = node.getnewaddress()
  auxblock = node.createauxblock (cb_addr)
...
def mineScryptBlock (header, target, ok):
  ...
  while True:
    ...
    #if (ok and int(scrypt, 16) < int(target, 16)) or ((not ok) and int(scrypt, 16) > int(target, 16)):
...
def getScryptPoW(hexData):
  ...
  hashed_data = hashlib.scrypt(data, salt=data, n=1024, r=1, p=1, maxmem=0, dklen=32)
  return reverseHex(hashed_data.hex())
