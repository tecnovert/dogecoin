...
class CreateAuxBlockTest(BitcoinTestFramework):
  ...
  def set_test_params(self):
    ...
    self.extra_args = [["-debug", "-txindex"], ["-debug", "-rpcnamecoinapi"]]
  ...
  def run_test(self):
    ...
    self.nodes[1].createwallet(wallet_name="test")

    addr0 = self.nodes[0].getnewaddress()
    addr1 = self.nodes[1].getnewaddress()

    # Generate an initial chain
    self.generatetoaddress(self.nodes[0], 100, addr0)
    self.sync_all()
    # Generate a block so that we are not "downloading blocks".
    self.generatetoaddress(self.nodes[1], 1, addr1)
    self.sync_all()
  ...
  def check_mined_block(self, auxblock, apow, addr, min_value, txid=None):
    ...
    assert_equal(coinbasetx["vout"][0]["scriptPubKey"]["address"], addr)
