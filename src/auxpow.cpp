...
bool checkAuxPow(CAuxPow &auxpow, const uint256& hashAuxBlock, int nChainId, const Consensus::Params& params)
{
    ...
    if (params.fStrictChainId && auxpow.parentBlock.GetChainId () == nChainId)
    ...
    const uint256 nRootHash
      = auxpow.CheckMerkleBranch(hashAuxBlock, auxpow.vChainMerkleBranch, auxpow.nChainIndex);
    std::vector<unsigned char> vchRootHash(nRootHash.begin(), nRootHash.end());
    std::reverse(vchRootHash.begin(), vchRootHash.end()); // correct endian

    // Check that we are in the parent block merkle tree
    if (auxpow.CheckMerkleBranch(auxpow.GetHash(), auxpow.vMerkleBranch, auxpow.nIndex)
          != auxpow.parentBlock.hashMerkleRoot)
    ...
    memcpy(&nSize, &pc[0], 4);
    nSize = le32toh(nSize);
    const unsigned merkleHeight = auxpow.vChainMerkleBranch.size();
    ...
    if (auxpow.nChainIndex != auxpow.getExpectedIndex (nNonce, nChainId, merkleHeight))
